import copy
from net.layer.fpr_nms import fpr_encode
import time
import random
import numpy as np
import torch
...
def make_one_fpr_target(cfg, input, proposal, truth_box, truth_label):
    ...
    if len(truth_box) == 0:
        num_bg = min(len(proposal), cfg['fpr_train_batch_size'])
    ...
    fg_index = np.where(max_overlap >= cfg['fpr_train_fg_thresh_low'])[0]
    bg_index = np.where(max_overlap <  cfg['fpr_train_bg_thresh_high'])[0]

    # sampling for class balance
    num_class = cfg['num_class']
    num = cfg['fpr_train_batch_size']
    num_fg = int(np.round(cfg['fpr_train_fg_fraction'] * cfg['fpr_train_batch_size']))
    ...
    if num_fg>0:
        ...
        sampled_target = fpr_encode(target_box, target_truth_box, cfg['box_reg_weight'])
...
def make_fpr_target(cfg, mode, inputs, proposals, truth_boxes, truth_labels):
    ...
    for b in range(batch_size):
        ...
        sampled_proposal, sampled_label, sampled_assign, sampled_target = \
           make_one_fpr_target(cfg, input, proposal, truth_box, truth_label)
