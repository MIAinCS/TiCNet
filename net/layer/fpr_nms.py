...
def fpr_encode(window, truth_box, weight):
    return box_transform(window, truth_box, weight)


def fpr_decode(window, delta, weight):
    return  box_transform_inv(window, delta, weight)


def fpr_nms(cfg, mode, inputs, proposals, logits, deltas):

    if mode in ['train',]:
        nms_pre_score_threshold = cfg['fpr_train_nms_pre_score_threshold']
        nms_overlap_threshold   = cfg['fpr_train_nms_overlap_threshold']

    elif mode in ['valid', 'test','eval']:
        nms_pre_score_threshold = cfg['fpr_test_nms_pre_score_threshold']
        nms_overlap_threshold   = cfg['fpr_test_nms_overlap_threshold']
        #
        # if mode in ['eval']:
        #     nms_pre_score_threshold = 0.05 # set low numbe r to make roc curve.

    else:
        raise ValueError('fpr_nms(): invalid mode = %s?'%mode)
        ...
        if len(index)>0:
            ...
            for j in range(1, num_class): #skip background
                ...
                if len(idx)>0:
                    ...
                    d = delta[idx, j]
                    box = fpr_decode(proposal[idx, 2:8], d, cfg['box_reg_weight'])
...
def get_probability(cfg, mode, inputs, proposals, logits, deltas):
    ...
    num_class = cfg['num_class']
    probs = F.softmax(logits, dim=1).cpu().data.numpy()
    # deltas = torch.from_numpy(deltas).cpu().data.numpy().reshape(-1, num_class, 6)
    deltas = deltas.cpu().data.numpy().reshape(-1, num_class, 6)
    proposals = proposals.cpu().data.numpy()
    ...
    for j in range(1, num_class):  # skip background
        ...
        if len(idx) > 0:
            ...
            d = deltas[idx, j]
            box = fpr_decode(proposals [idx, 2:8], d, cfg['box_reg_weight'])
            box = clip_boxes(box, inputs.shape[2:])
            js = np.expand_dims(np.array([j] * len(p)), axis=-1)
            output = np.concatenate((p, box, js), 1)
