...
def fpr_loss(logits, deltas, labels, targets, deltas_sigma=1.0):
    ...
    fpr_cls_loss = F.cross_entropy(logits, labels, weight=weight, size_average=True)
    ...
    if num_pos > 0:
        ...
        fpr_reg_loss = Variable(torch.cuda.FloatTensor(1).zero_()).sum()

    return fpr_cls_loss, fpr_reg_loss




def fpr_iou_loss(rpn_proposals, truth_box):
    iou_loss = 0
